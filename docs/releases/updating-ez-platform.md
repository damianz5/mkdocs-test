...
If you get a **lot** of conflicts (on the `doc` folder for instance), and eZ Platform was installed from the [share.ez.no](http://share.ez.no) tarball, it might be because of incomplete history. You will have to run `git fetch ezplatform --unshallow` (or `git fetch ezstudio --unshallow`) to load the full history, and run the merge again.
...
```
* * * * *   (cd /path/to/your/ezstudio-project && app/console ezpublish:cron:run -e=prod)
```

For development environment:

```
* * * * *   (cd /path/to/your/ezstudio-project && app/console ezpublish:cron:run -e=dev)
```
