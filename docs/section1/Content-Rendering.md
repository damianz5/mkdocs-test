...
Created by Dominika Kurek, last modified on Feb 09, 2017
...
The example above can be translated as "Match any content which **ContentType** identifier is ***small\_folder* OR *folder*** , **AND** having *frontpage* as **ParentContentType** identifier".
...
For more information about the **view provider configuration**, [please refer to the dedicated section](#ContentRendering-Viewproviderconfiguration)  .

You can also [use your own custom controller to render a content item/location](#ContentRendering-Customcontrollers).
...
Having access to the Content item in the template, you can use [its public methods](https://github.com/ezsystems/ezpublish-kernel/blob/master/eZ/Publish/Core/Repository/Values/Content/Content.php) to access all the information you need. You can also use the **ez\_field\_value** helper to get the [field types reference](Field-Types-reference_31430495.html) . It will return the correct language if there are several, based on language priorities.
...
Controller selection doesn't apply to `block_view` since you can already [use your own controller to display blocks](https://doc.ez.no/display/DEVELOPER/Content+Rendering#ContentRendering-Renderblock).
...
-   [Usage](#ContentRendering-Usage)
    -   [View selection](#ContentRendering-Viewselection)
    -   [Content view templates](#ContentRendering-Contentviewtemplates)
    -   [Custom controllers](#ContentRendering-Customcontrollers)
    -   [Embedded images](#ContentRendering-Embeddedimages)
    -   [Query controller](#ContentRendering-Querycontroller)
    -   [Making links to other locations](#ContentRendering-Makinglinkstootherlocations)
    -   [Render embedded content items](#ContentRendering-Renderembeddedcontentitems)
    -   [Render block](#ContentRendering-Renderblock)
